...
$('.loginbox').show();
RegisterLoginComponent.centerRegLoginForm(400,360);
$(window).resize(function(){
    RegisterLoginComponent.centerRegLoginForm(400.360);
});

var input_mobile = RegisterLoginComponent.initInputMobile({defaultPlaceHolder:'您的手机号',focusPlaceHolder:'您的手机号',existCheck:false});
var input_imageVerifyCode = RegisterLoginComponent.initInputImageVerifyCode();
input_imageVerifyCode.e.keydown(function(e){
    if (e.keyCode == 13) {
        $("#get-code-btn").trigger('click');
    }
});
//var input_mobileActivateCode = RegisterLoginComponent.initInputMobileActiveCode();
var input_password = RegisterLoginComponent.initInputPassWord({defaultPlaceHolder:'新密码',focusPlaceHolder:'6-12个字母、数字或符号，且至少包含两种'});
input_password.e.keydown(function(e){
    if (e.keyCode == 13) {
        $("#pwdmoile-btn").trigger('click');
    }
});
var oriMobileNum = '1';
...
input_mobileActivateCode.e.keyup(function(){
    ...
    $('#activate-code').removeClass('r_input_small_cur');
}).blur(function(){
    var activateCode = $(this).val();
    var checker = checkactivateCode(activateCode);
    if (checker.rst == false) {
        $('#activate-code').addClass('r_input_small_cur');
        input_mobileActivateCode.showError(checker.msg);
        return true;
    }
});

function initGetCodeBtn (){
    //{{{

    var getcode_btn = sendCodeButton('#quick-get-code-btn',{second:120});
    getcode_btn.state = false;
    //input_imageVerifyCode.codeSendSuc = true;
    getcode_btn.start(120);
    $('#quick-get-code-btn').click(function(){
        if (getcode_btn.state == false) {
            input_imageVerifyCode.status = false;
            oriMobileNum = '1';
            $("#findpasswordmobile2").hide();         
            $('#input-mobile').next().show();
            $('#input-mobile').next().removeAttr('style');
        }
    });
...
function checkActiveCode(){
    ...
    ajaxCheck = pub.checkConfirmCode(input_mobileActivateCode.e.val(),input_mobile.e.val(),nationName,nationCode);
...
var setButtonText = function (buttonText) {
...
$('#pwdmoile-btn').click(function(){
    if(!input_mobileActivateCode.status){
        input_mobileActivateCode.e.blur();
        checkActiveCode();
    }
    if(!input_password.status){
        input_password.e.blur(); 
    }
    if (!(input_mobile.status && input_mobileActivateCode.status && input_password.status)){
        return false;
    }
    
    var nationName = $('.nation-cont i').attr('tag');
    var nationCode = $('.nation-cont i').attr('code');
    var reg_rst = pub.findPasswordByMobile(
        input_mobile.e.val(),
        nationName,
        nationCode,
        input_mobileActivateCode.e.val(),
        input_password.e.val()
        );
    if (reg_rst.status == 0){
        alert(reg_rst.errmsg);
    }else{
        window.location.href = reg_rst.sucmsg; 
    }
});
$('#input-mobile').blur(function(){
    if(input_mobile.status == false) return false;
    var nationName = $('.nation-cont i').attr('tag');
    var nationCode = $('.nation-cont i').attr('code');
    var ajaxChecker = pub.mobileExist($('#input-mobile').val(),nationName,nationCode);
    if(ajaxChecker.status == 1) {
        input_mobile.showError("手机号码未注册");
        input_mobile.e.addClass('r_input_1_cur');
        return false;
    }

});
$('#get-code-btn').live('click', function(){
    ...
    $.when((function(){
        ...
        if (input_mobile.status == false) return false;
        if(input_mobile.e.val() == oriMobileNum){
            $("#findpasswordmobile2").show();
            return false;
        }

        var nationName = $('.nation-cont i').attr('tag');
        var nationCode = $('.nation-cont i').attr('code');
        var ajaxSender = pub.sendConfirmCode(input_imageVerifyCode.e.val(), input_mobile.e.val(),nationName,nationCode);
        if (ajaxSender.status == 0){
            input_imageVerifyCode.showError(ajaxSender.errmsg)
            return false;
        }
        $('.region,.nation-num').hide();
        oriMobileNum = $("#input-mobile").val();
        var inputMobile = $('.nation-cont i').text() + ' ' + $("#input-mobile").val();
        $("#mobilePhone").html(inputMobile);
        $("#findpasswordmobile2").show();
        $('#region').hide();
        initGetCodeBtn();
    });

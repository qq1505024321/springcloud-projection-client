 ...
 $(function(){
    ...
    input_username.e.blur(function(){
        if($(this).val() == '')
        {
            ...
            if($('#password').val() != ''){
                var nationName = $('.region').is(":hidden") ? '' : $('.nation-cont i').attr('tag');
                var nationCode = $('.region').is(":hidden") ? '' : $('.nation-cont i').attr('code');
                XZWebAjax.postSpider('',true,XZWebUrlWriter.getAjax_CheckRegistExist(),{user:$('#input-username').val(),nationName:nationName,nationCode:nationCode},true,function(userRegist){
                    var errColor = $('.region').is(":hidden") ? 'r_input_1_cur' : 'r_input_small_cur';
                    if(userRegist.status == 0){
                        input_username.e.addClass(errColor);
                        input_password.showError(userRegist.errmsg);
                        return false;
                    }
                    input_username.e.removeClass(errColor);
    ...
    $("#orgBtn").click(function(){
        ...
        if(cookieApi.getCookie('isAutoTest') !== 'yes' &&
            localStorage.getItem('SAFE_CHECKED_login') === 'no'){
            $("#input-slidecode-tip2").css('display','block');
            $("#input-slidecode-tip5").css('display','block');
            verifyState = false;
        }
        var spiderAvoidToken= localStorage.getItem('SPIDER_AVOID_TOKEN_login');
        $.when((function(){
            var def = $.Deferred();
            ...
            if(!(input_username.status && input_password.status && verifyState))
            {
                return false;
            }
            var setcookie =  0;
            ...
            {username:username, password:password, spiderAvoidToken:spiderAvoidToken, setcookie:setcookie,lodgeid:lodgeid,nationName:nationName,nationCode:nationCode},
            true,
            function(returndata){
    ...
    window.XZWebMobileQuickLogin = {
        ...
        initMobileBtn : function(){
            ...
            $("#orgBtnMobile").click(function(e){
                ...
                if(cookieApi.getCookie('isAutoTest') !== 'yes' &&
                    localStorage.getItem('SAFE_CHECKED_login') === 'no'){
                    $("#input-slidecode-tip3").css('display','block');
                    $("#input-slidecode-tip4").css('display','block');
                    return
                }
                that.hasCommit = true;
                $.when((function(){
        ...
        initGetCodeBtn : function(){
            ...
            getcode_btn.e.click(function(){
                $.when((function(){
                    ...
                    if (that.input_mobile.status && (getcode_btn.state == false)){
                        that.getActiveCode();
                    }
                });
            });
        ...
        getActiveCode : function(){
            ...
            var ajaxRsponse =  pub.sendQuickLoginCode($(".nation-cont i").attr('tag'),$(".nation-cont i").attr('code'), this.input_mobile.e.val());
        ...
        loginByMobile : function(){
            ...
            var setcookie =  0;
            var lodgeid=$.trim($("#lodgeid").val());
            if($('#quick-activate-code-tip:visible').length){
                return
            }

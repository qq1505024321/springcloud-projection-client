...
function Captcha(config) {
 ...
 var defaultHeight = $('div[class="guide_sider"]').offset().top;
$(function(){
...
{
    $("#menu_"+index).children("span").addClass('circle_now');
    $("#part_"+index).addClass('column_now');
}
function mouseOut(index)
{
    if($("#part_more_"+index).is(":hidden"))
    {
        $("#menu_"+index).children("span").removeClass('circle_now');
        $("#part_"+index).removeClass('column_now');
    }
}
function togglePart(index)
{
    ...
    $("html,body").animate({scrollTop: $("#part_"+index).offset().top}, 1000);
    resetHeight();
}
function showMore(index)
{
    var guidetype = index.split('_')[0];
    var idx = 0;
    $('dl[class="guide_dl"]').each(function(i){
        hiddenMore(guidetype+'_'+(i+1));
    });
    $("#menu_"+index).children("span").addClass('circle_now');
    $("#part_"+index).addClass('column_now');
    $("#sub_menu_"+index).show();
    $("#part_more_"+index).show();
    $("#more_"+index).hide();
}
function hiddenMore(index)
{
    $("#menu_"+index).children("span").removeClass('circle_now');
    $("#part_"+index).removeClass('column_now');
    $("#sub_menu_"+index).hide();
    $("#part_more_"+index).hide();
    $("#more_"+index).show();
}
function resetHeight()
{
    if( $(document).scrollTop() >= defaultHeight )
    {
        $('div[class="guide_sider"]').css("top","0px");
        $('div[class="guide_sider"]').css("z-index","100");
        $('div[class="guide_sider"]').css("position","fixed");
    }
    else {
        $('div[class="guide_sider"]').removeAttr('style');
    }

}
function changeGuide(guideType)
{
    $("ul[class='guide_tabs'] li").removeClass('tabs_current');
    $("#tab_"+guideType).addClass('tabs_current');
    $("div[name='left_guide']").hide();
    $("#left_"+guideType+"_guide").show();

    $("div[name='right_guide']").hide();
    $("#right_"+guideType+"_guide").show();
    $.ajax({
        type : "GET",
        url : XZWebUrlWriter.getAjax_IncreaseGuideVisits(guideType),
